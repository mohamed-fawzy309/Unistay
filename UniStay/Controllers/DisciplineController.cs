...
public class DisciplineController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> BulkPenalty()
    {
        ViewBag.Cities = await _db.DormitoryCities
            .Where(c => c.IsActive && !c.IsDeleted)
            .Select(c => new SelectListItem { Value = c.ID.ToString(), Text = c.Name })
    ...
    public async Task<IActionResult> BulkPenalty(BulkPenaltyViewModel model)
    {
        if (!ModelState.IsValid)
        {
            ViewBag.Cities = await _db.DormitoryCities
        ...
        var now = DateTime.UtcNow;
        var violations = students.Select(s => new Violation
        {
            StudentID = s.ID,
            DormitoryCityID = model.DormitoryCityID,
        ...
        await _audit.LogAsync(CurrentUserId, "Staff", "Violation.BulkAdd",
            "Violation", null, null,
            new { model.ViolationType, model.Severity, StudentCount = violations.Count, model.DormitoryCityID });

        TempData["Success"] = $"تم تسجيل {violations.Count} مخالفة بنجاح";
        return RedirectToAction("BulkPenalty");
    }
}
